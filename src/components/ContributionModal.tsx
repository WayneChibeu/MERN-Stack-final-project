...
const ContributionModal: React.FC<ContributionModalProps> = ({ project, isOpen, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (contributionData.type === 'monetary') {
  ...
  const submitContribution = async (transactionCode?: string) => {
    if (!project) return;
    ...
    try {
      const contributionPayload = {
        ...
        payment_method: transactionCode ? 'manual_mpesa' : 'none'
      };
      ...
      showToast(
        transactionCode
          ? `Thank you! Your donation to "${project.title}" is pending approval.`
          : `Thank you for your ${contributionData.type} contribution to "${project.title}"!`,
        'success'
      );
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="flex space-x-3 pt-4">
              ...
              >
                ...
                <span>{contributionData.type === 'monetary' ? 'Donate Now' : 'Submit Contribution'}</span>
              </button>
            </div>
          </form>
        </div>
